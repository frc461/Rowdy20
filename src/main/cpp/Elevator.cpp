...
RobotElevator::RobotElevator(Control *control) {
    ...
    idealPosition = Control::ElevatorPosition::BallTower0;
...
void RobotElevator::RunMotorSafe(double speed) {
    if(speed == 0){
        brake->Set(1); //Apply the brake if we are not going down
        Elevator0->Set(0);
    }else {
